...
from iam_smoke.tester import assume_role_session, get_current_account_id, get_role_arn
...
def test_iam_policy_modify_should_fail():
    ...
    try:
        ...
        response = iam.delete_policy_version(PolicyArn=policy_arn, VersionId="v1")
        print(f"Unexpected success in delete_policy_version: {response}")
        pytest.fail(
            f"delete_policy_version unexpectedly succeeded with response: {response}"
        )
    ...
    try:
        ...
        pytest.fail(
            f"create_policy_version unexpectedly succeeded with response: {response}"
        )
    except ClientError as e:
        ...
        if error_code == "LimitExceededException":
            print(
                "Handling LimitExceededException by trying to delete an older version first"
            )
            ...
            try:
                ...
                print(
                    f"Unexpected success in list_policy_versions: {versions_response}"
                )
                pytest.fail(
                    f"list_policy_versions unexpectedly succeeded with response: {versions_response}"
                )
            ...
            print(
                f"Attempting to delete specific policy version v2 for ARN: {policy_arn}"
            )
            try:
                ...
                print(
                    f"Unexpected success in delete_policy_version v2: {delete_response}"
                )
                pytest.fail(
                    f"delete_policy_version v2 unexpectedly succeeded with response: {delete_response}"
                )
            ...
            print(
                "Attempting to create policy version again after handling LimitExceededException"
            )
            try:
                ...
                print(
                    f"Unexpected success in create_policy_version retry: {create_again_response}"
                )
                pytest.fail(
                    f"create_policy_version retry unexpectedly succeeded with response: {create_again_response}"
                )
            except ClientError as create_again_e:
                print(
                    f"Expected error in create_policy_version retry: {create_again_e}"
                )
        else:
            # Verify it's an access denied error for the initial create attempt
            assert any(
                error in str(e) for error in ["AccessDenied", "AccessDeniedException"]
            )
    ...
    try:
        ...
        print(
            f"Note: get_policy failed but this is not our primary test focus: {get_e}"
        )
