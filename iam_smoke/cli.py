...
def setup_command(args):
    ...
    if args.bucket:
        setup_test_bucket(region=args.region)
...
def teardown_command(args):
    ...
    if args.role:
        success = delete_test_role(region=args.region)
        if success:
...
def main():
    ...
    setup_parser.add_argument(
        "--role", action="store_true", help="Create test IAM role"
    )
    setup_parser.add_argument(
        "--permission-boundary", help="ARN of permission boundary to apply to the role"
    )
    setup_parser.add_argument(
        "--clone-from-role",
        help="Name of an existing role to clone policies and boundary from",
    )
    setup_parser.add_argument(
        "--bucket", action="store_true", help="Create test S3 bucket"
    )
    setup_parser.add_argument(
        "--region", default=DEFAULT_REGION, help="AWS region for resources"
    )
    ...
    teardown_parser.add_argument(
        "--role", action="store_true", help="Delete test IAM role"
    )
    teardown_parser.add_argument(
        "--region", default=DEFAULT_REGION, help="AWS region for resources"
    )
