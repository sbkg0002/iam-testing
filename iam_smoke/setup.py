    ...
    if clone_from_role:
        ...
        try:
            ...
            if "PermissionsBoundary" in role_data:
                cloned_boundary_arn = role_data["PermissionsBoundary"].get(
            ...
            for policy_name in policy_names:
                policy_response = iam.get_role_policy(
                    RoleName=clone_from_role, PolicyName=policy_name
                )
            ...
            managed_policies_response = iam.list_attached_role_policies(
        ...
        if clone_from_role and "managed_policy_arns" in locals():
            for policy_arn in managed_policy_arns:
                try:
                    ...
                    logger.info(
                        f"Attached managed policy {policy_arn} to role {role_name}"
                    )
                except ClientError as e:
                    logger.warning(
                        f"Failed to attach managed policy {policy_arn}: {str(e)}"
                    )
...
def delete_test_role(
    role_name: str = DEFAULT_ROLE_NAME, region: str = DEFAULT_REGION
) -> bool:
    """
...
def setup_test_bucket(
    bucket_name: str = DEFAULT_TEST_BUCKET, region: str = DEFAULT_REGION
) -> str:
    """
    ...
    try:
        ...
        if error_code == "403" or error_code == "404" or error_code == "NoSuchBucket":
            ...
            if error_code == "403":
                logger.warning(
                    f"Bucket {bucket_name} exists but is not accessible, creating a unique bucket"
                )
                # Create a unique name based on original + account ID + random suffix
                random_suffix = uuid.uuid4().hex[:8]
                bucket_name = (
                    f"{original_bucket_name}-{account_id}-{random_suffix}".lower()
                )
            ...
            try:
                ...
                logger.info(
                    f"Added test object 'restricted.txt' to bucket {bucket_name}"
                )
                return bucket_name
            except ClientError as e:
